...
public class OsChinaSpider implements PageProcessor{
	...
	static List<Object> hrefs = Lists.newArrayList();
	...
	public void process(Page page) {
		if (page.getUrl().regex("^https://www\\.oschina\\.net/blog").match()) {
		...
		} else if(page.getUrl().regex(URL_LIST).match() && pageNum <= 2){
			try {
			...
			System.out.println("标题为："+page.getHtml().xpath("//div[@id='blogBody']").get());
		}
	}
